      ...
      <Fragment>
        ...
        {loading ? (
          <Stack alignItems="center" justifyContent="center" style={{ marginTop: '300px' }}>
            ...
            <>
              {posts.map((post: any) => (
                <PostCard key={post.id} id={post.id} user_id={post.user_id} text={post.text} image={post.image} title={post.title} created_at={post.created_at} username={post.userName} userimage={post.userImage} isfavorited={post.favorited} />
              ))}
            </>
          )}
      </Fragment>
